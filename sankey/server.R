...
data_full<-dbGetQuery(getConnection(),"
		select type source, case when  a.status = 'Failed Pre-Screen' then dq_reason else 'Post-Screen' end as target, count(distinct patient_id) counts from gidb.endo1 a
		...
		union
		select 'Post-Screen' as source, case when  a.status like 'Excluded%' then dq_reason else 'Site-Screen' end as target, count(distinct patient_id) counts from gidb.endo1 a
		join gidb.`dim_media_type` b on a.media_type = b.detail
		...
		union 
		select 'Site-Screen' as source, if(status = 'Randomized', status, if(el_param is not null and el_param <> '',el_param, 'Unknown')) as target, count(distinct concat(initials,screen)) counts from gidb.eligible_track a
...
data<-reactive({
	...
	if(input$media_group=='types'){
		b<-ddply(a,~source,summarise,target=target,value=counts/sum(counts))}
	else if(input$media_group=='group')
	{
		b<-ddply(a,~target,summarise,source='Inquiry',value=sum(counts))
}
